...
function migrateLayout(cwd, answers) {
  ...
  for (const [from, to] of project.layoutMoves(answers, cwd)) {
  ...
  const active = path.join(cwd, answers.docs, 'tasks/active');
  if (fs.existsSync(active) && isEmptyDir(active)) fs.rmdirSync(active);
