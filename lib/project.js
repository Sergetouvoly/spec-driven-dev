...
//
// The docs root holds specs and tasks only. Everything else is filed by what
// it is for: reference/ is read (vocabulary, decisions), workflow/ is
...
function rulesBlock(a) {
  ...
  const lines = [
    ...
    '## Workflow',
    '',
    `Features go through /spec then /task before implementation. See ${d}/workflow/status.md`,
    ...
    '',
    `Tasks live in ${d}/tasks/<spec-slug>/, one folder per spec, ad-hoc ones`,
    'in adhoc/.',
    '',
    `Never read ${d}/archive/: it holds finished tasks, a human trail rather than`,
    'context.',
    '',
    `Every other document is filed by purpose: ${d}/reference/ for what you read`,
    `(CONTEXT.md, adr/), ${d}/workflow/ for what the commands generate (status.md).`,
    '',
...
function docsDirs(a) {
  return ['specs', 'tasks', 'archive/tasks', 'reference/adr', 'workflow'].map((d) => `${a.docs}/${d}`);
}

const contextPath = (a) => `${a.docs}/reference/CONTEXT.md`;
const statusPath = (a) => `${a.docs}/workflow/status.md`;

// The earlier layout kept these at the docs root, and open tasks one level
// down in tasks/active/. update moves them.
function layoutMoves(a, cwd) {
  const active = `${a.docs}/tasks/active`;
  let folders = [];
  try {
    folders = fs.readdirSync(path.join(cwd, active)).sort();
  } catch {
    // no earlier layout
  }
  return [
    [`${a.docs}/CONTEXT.md`, contextPath(a)],
    [`${a.docs}/adr`, `${a.docs}/reference/adr`],
    [`${a.docs}/status.md`, statusPath(a)],
    ...folders.map((f) => [`${active}/${f}`, `${a.docs}/tasks/${f}`]),
  ];
}
...
module.exports = { rulesBlock, withRulesBlock, withSpecloopHooks, contextStarter, statusStarter, docsDirs, contextPath, statusPath, layoutMoves, topFolders, START, END };
