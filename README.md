...
Seven artifacts, and each answers a question none of the others do: the spec what
the product must do, the task what to build next, `status.md` where things stand
right now, `MAP.md` where code lives, `CONTEXT.md` what the words mean, the ADRs
what was decided and cannot be cheaply undone, `CHANGELOG.md` what a user got.
Git holds the detailed history under all of it.
