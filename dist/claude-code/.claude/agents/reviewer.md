...
- Tool access is granted by the `tools` field above. It is a whole-tool allowlist: a tool listed there is usable on any path. Where a restriction stated in this file is narrower than the tool, it is a rule you follow, not a wall the runtime enforces.
- A `subagent` agent is invoked through the Task tool; a `primary` agent is one you adopt in an interactive session, never as a subtask.
