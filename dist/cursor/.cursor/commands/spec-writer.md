...
- Cursor cannot enforce these limits. The restrictions in this file are rules you follow, not walls. Nothing stops a tool call that breaks one.
- Cursor has no subagent. Run this file as its own command in a fresh chat, so it is read without the reasoning that produced the diff.
